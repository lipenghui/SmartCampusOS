...
> ⚠️ **pnpm 不是 Node 自带命令**。若执行 `pnpm ...` 提示“无法识别/command not found”，说明 pnpm 未全局安装——本文档统一使用 **`corepack pnpm ...`** 前缀（corepack 随 Node 分发，会按 `package.json` 的 `packageManager` 自动使用正确版本），无需额外安装。
>
> 可选：执行 `corepack enable pnpm`（或 `npm i -g pnpm@9.15.9`）后即可直接使用裸 `pnpm` 命令。
...
corepack pnpm --filter @smartcampus/admin dev

# 类型检查（shared + admin）
corepack pnpm -r typecheck

# 生产构建
corepack pnpm --filter @smartcampus/admin build
```
