...
**入口**:`02-Backend/Services/IdentityService/tests/IdentityService.FunctionalTests/`
**架构**:真实环境(不走 TestHost)——`04-scripts/start-e2e-env.sh` 编排 MySQL/Redis 容器 + IdentityService(localhost:5111)+ ApiGateway(localhost:5000),测试经网关走**真实契约**(JWT 校验、X-* 头透传、Redis 黑名单)。
...
**入口**:`03-Frontend/e2e/`(独立包,@playwright/test 1.62.1 + playwright-bdd 9.2.0,中文 Gherkin)
...
- `04-scripts/start-e2e-env.sh`:幂等启动 MySQL/Redis 容器 + IdentityService(5111)+ ApiGateway(5000),端到端探活(登录 admin → 200)后返回;退出码非 0 报错。
- `04-scripts/stop-e2e-env.sh`:停止服务进程并移除容器。
...
cd 03-Frontend/e2e && npm install && npx playwright install chromium
