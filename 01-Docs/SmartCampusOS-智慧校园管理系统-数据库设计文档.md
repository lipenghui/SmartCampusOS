...
| 编写日期 | 2026-08-06 |
| 编写人 | 架构组 / DBA |
| 关联文档 | SmartCampusOS-智慧校园管理系统-PRD v1.0、SmartCampusOS-智慧校园管理系统-LLD v1.1 |
| 适用对象 | 研发、测试、DBA、运维 |
...
| v1.1 | 2026-08-06 | 架构组 | 现状对齐：标注各库实现状态（✅/🚧）；identity_db 字段与 `IdentityService.Domain/Entities` 核对修正；附录表总览加状态列 |

> **权威性说明**：本文档为 SmartCampusOS 数据表结构与 ER 关系的**唯一权威来源**。LLD v1.1 不再维护数据表细节，两者冲突时以本文档为准。
>
> **实现状态图例**：✅ = 已实现（表结构与 `src/<X>.Domain/Entities` 核对一致）；🚧 = 规划中（表结构为设计基线，代码尚未落地，随对应服务实现）。已实现库（identity_db）的字段以代码实体为准（见 §3 字段核对说明）。
...
- [1. 引言](#1-引言)
- [2. 设计约定](#2-设计约定)
- [3. IdentityService（identity_db）— 12 表 ✅](#3-identityserviceidentity_db-12-表-)
- [4. EduService（edu_db）— 14 表 🚧](#4-eduserviceedu_db-14-表-)
- [5. DormService（dorm_db）— 10 表 🚧](#5-dormservicedorm_db-10-表-)
- [6. NoticeService（notice_db）— 8 表 🚧](#6-noticeservicenotice_db-8-表-)
- [7. DataService（data_db）— 3 表 🚧](#7-dataservicedata_db-3-表-)
- [8. PushGateway（push_db）— 2 表 🚧](#8-pushgatewaypush_db-2-表-)
- [9. FileService（file_db）— 1 表 🚧](#9-fileservicefile_db-1-表-)
- [10. 跨服务数据引用与一致性约定](#10-跨服务数据引用与一致性约定)
- [11. 索引与性能要点](#11-索引与性能要点)
- [12. 数据字典（状态/枚举值汇总）](#12-数据字典状态枚举值汇总)
- [13. 变更管理](#13-变更管理)
- [附录：表总览（50 表）](#附录表总览50-表)
...
## 3. IdentityService（identity_db）— 12 表 ✅ 已实现
...
> **字段核对（v1.1）**：下表已与代码实体 `IdentityService.Domain/Entities`（`sys_` 表，共 12 个实体）逐表核对。以下差异**以代码实体为准**：
> - `sys_user`：手机号拆分为 `mobile_encrypted`（AES-256 密文）+ `mobile_hash`（检索哈希）；密码盐字段名为 `password_salt`；`last_login_at` 为预留字段（代码暂未实现）；
> - `sys_org`：`sort`、`status` 为预留字段（代码暂未实现）；`sys_role.remark`、`sys_permission.type` 同理；
> - `sys_audit_log`：`target_id` 为 VARCHAR(64)（非 BIGINT）；`user_agent` 为预留字段；
> - `sys_dict_item`：启用状态以 `enabled`（bool）表达（文档 `status` 语义等价）；`sys_dict.remark` ↔ 代码 `description`。
...
| mobile_encrypted | VARCHAR(255) | 是 | NULL | 手机号密文（AES-256 加密存储） |
| mobile_hash | VARCHAR(64) | 是 | NULL | 手机号哈希（用于唯一性/登录检索，不存明文） |
...
索引：`uk_user_no`(user_no)、`idx_mobile_hash`(mobile_hash)。
...
| enroll_date | DATE | 是 | NULL | 入学日期（代码实体为可空） |
...
| revoked_at | DATETIME | 是 | NULL | 吊销时间 |
...
| target_id | VARCHAR(64) | 是 | NULL | 目标 ID |
...
## 5. DormService（dorm_db）— 10 表 🚧 规划中
...
## 6. NoticeService（notice_db）— 8 表 🚧 规划中
...
*本文档为 SmartCampusOS v1.1 数据库设计基线，变更须经 DBA/架构评审并更新修订记录；建表脚本由 FreeSql CodeFirst 生成，不另行维护手工 DDL。已实现库（identity_db）字段以代码实体为准，规划中库（edu_db / dorm_db / notice_db / data_db / push_db / file_db）以本文档为设计基线。*
