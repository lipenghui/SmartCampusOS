...
router.beforeEach(async (to) => {
  ...
  // 首次进入：用户信息未加载或动态路由未注册时，拉取用户信息并注册动态路由。
  // 注意：登录成功（applyAuth）已写入 userInfo，此时 userInfo 非空但动态路由尚未注册，
  // 必须以 dynamicRoutes 为空作为注册条件，否则所有动态路由页面都会落 404。
  if (!userStore.userInfo || permissionStore.dynamicRoutes.length === 0) {
    try {
