...
public sealed class DbSeeder
{
    ...
    {
        if (_fsql.Select<Permission>().Any())
        {
            return;
        }

        var permissions = PermissionDefs
            .Select((p, i) => new Permission { Id = i + 1, Code = p.Code, Name = p.Name, Module = p.Module })
            .ToList();
        _fsql.Insert(permissions).ExecuteAffrows();
        _logger.LogInformation("已初始化权限点 {Count} 个", permissions.Count);
    }
