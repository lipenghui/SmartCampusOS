...
/// 首次使用时自动执行 04-scripts/start-e2e-env.sh 确保环境就绪(可重复执行、幂等)。
/// </summary>
public static class Env
{
    ...
    /// <summary>SmartCampusOS 仓库根目录(含 04-scripts/ 与 02-Backend/)。</summary>
    ...
    public static void EnsureReady()
    {
        ...
        if (!proc.WaitForExit(TimeSpan.FromMinutes(8)))
        {
            ...
            throw new TimeoutException("功能测试环境启动超时(8 分钟), 请查看 04-scripts/logs/*.log");
        }

        if (proc.ExitCode != 0)
        {
            throw new InvalidOperationException($"环境启动失败(exit {proc.ExitCode}), 请查看 04-scripts/logs/*.log");
        }
    }
    ...
    private static string FindRepoRoot()
    {
        ...
        throw new DirectoryNotFoundException("未找到 SmartCampusOS 仓库根目录(含 04-scripts/ 与 02-Backend/)");
    }
}
