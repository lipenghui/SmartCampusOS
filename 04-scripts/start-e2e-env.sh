...
# 启动:04-scripts/start-e2e-env.sh   停止:04-scripts/stop-e2e-env.sh
set -euo pipefail
...
if curl -sf "http://localhost:$GATEWAY_PORT/health" >/dev/null 2>&1; then
  ...
  (cd "$ROOT/02-Backend/ApiGateway/src/ApiGateway" && \
  ...
  sleep 3
  echo "    启动中, 日志: 04-scripts/logs/gateway.log"
fi
...
echo "    查看日志: 04-scripts/logs/identity.log / 04-scripts/logs/gateway.log" >&2
exit 1
